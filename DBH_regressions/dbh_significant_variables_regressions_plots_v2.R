...
ggplot(data_long_signif, aes(x = dbh_cm, y = Value, color = TreeSpecies)) +
 ...
 # geom_smooth(method = "loess", se = TRUE, linetype = "solid", color = "black") +  # LOESS trend line with confidence intervals
  stat_poly_eq(aes(label = paste(..rr.label.., ..p.value.label.., sep = "~~~")), # plot R2 and p value on the graph
...
ggsave("DBH_regressions/Regression_Plots_DBH_vs_Signif_Variables_Faceted_Stats.png", width = 14, height = 8, dpi = 300)
